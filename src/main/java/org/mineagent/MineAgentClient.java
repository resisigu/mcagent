...
public final class MineAgentClient {
 ...
 private static void applyControl(Minecraft mc){Control c=control;mc.options.keyUp.setDown(c.forward>.1);mc.options.keyDown.setDown(c.forward<-.1);mc.options.keyLeft.setDown(c.strafe<-.1);mc.options.keyRight.setDown(c.strafe>.1);mc.options.keyJump.setDown(c.jump);mc.options.keySprint.setDown(c.sprint);mc.options.keyAttack.setDown(c.attack);
 if(c.attack&&attackCooldown<=0){mc.startAttack();attackCooldown=4;}mc.options.keyUse.setDown(c.use);LocalPlayer p=mc.player;if(p!=null){if(Float.isFinite(c.yaw))p.setYRot(c.yaw);if(Float.isFinite(c.pitch))p.setXRot(Math.max(-90f,Math.min(90f,c.pitch)));p.setSprinting(c.sprint);}}
