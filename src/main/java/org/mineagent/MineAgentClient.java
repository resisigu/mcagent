...
public final class MineAgentClient {
 ...
 private static void applyControl(Minecraft mc){Control c=control;mc.options.keyUp.setDown(c.forward>.1);mc.options.keyDown.setDown(c.forward<-.1);mc.options.keyLeft.setDown(c.strafe<-.1);mc.options.keyRight.setDown(c.strafe>.1);mc.options.keyJump.setDown(c.jump);mc.options.keySprint.setDown(c.sprint);mc.options.keyAttack.setDown(c.attack);LocalPlayer p=mc.player;if(p!=null){if(Float.isFinite(c.yaw))p.setYRot(c.yaw);if(Float.isFinite(c.pitch))p.setXRot(Math.max(-90f,Math.min(90f,c.pitch)));p.setSprinting(c.sprint);}}
 private static void releaseControls(Minecraft mc){mc.options.keyUp.setDown(false);mc.options.keyDown.setDown(false);mc.options.keyLeft.setDown(false);mc.options.keyRight.setDown(false);mc.options.keyJump.setDown(false);mc.options.keySprint.setDown(false);mc.options.keyAttack.setDown(false);}
 private static void recordLearning(Minecraft mc,LocalPlayer p){if(datasetFile==null)return;JsonObject o=baseState(mc,p);o.addProperty("forward",mc.options.keyUp.isDown()?1:(mc.options.keyDown.isDown()?-1:0));o.addProperty("strafe",mc.options.keyRight.isDown()?1:(mc.options.keyLeft.isDown()?-1:0));o.addProperty("jump",mc.options.keyJump.isDown());o.addProperty("sprint",mc.options.keySprint.isDown());o.addProperty("attack",mc.options.keyAttack.isDown());try{Files.writeString(datasetFile,o+System.lineSeparator(),StandardCharsets.UTF_8,StandardOpenOption.CREATE,StandardOpenOption.APPEND);}catch(IOException ignored){}}
 private static JsonObject baseState(Minecraft mc,LocalPlayer p){JsonObject o=new JsonObject();o.addProperty("time",System.currentTimeMillis());o.addProperty("x",p.getX());o.addProperty("y",p.getY());o.addProperty("z",p.getZ());o.addProperty("vx",p.getDeltaMovement().x);o.addProperty("vy",p.getDeltaMovement().y);o.addProperty("vz",p.getDeltaMovement().z);o.addProperty("yaw",p.getYRot());o.addProperty("pitch",p.getXRot());o.addProperty("onGround",p.onGround());o.addProperty("heldItem",BuiltInRegistries.ITEM.getKey(p.getMainHandItem().getItem()).toString());o.add("mobs",nearbyMobs(p,12));o.add("nearby",nearbyBlocks(p));return o;}
 private static JsonArray nearbyMobs(LocalPlayer p,double radius){JsonArray a=new JsonArray();for(Entity e:p.level().getEntities(p,p.getBoundingBox().inflate(radius),e->e instanceof Monster && e.isAlive())){Vec3 d=e.position().subtract(p.position());JsonObject m=new JsonObject();m.addProperty("id",e.getId());m.addProperty("type",BuiltInRegistries.ENTITY_TYPE.getKey(e.getType()).toString());m.addProperty("x",d.x);m.addProperty("y",d.y);m.addProperty("z",d.z);m.addProperty("distance",Math.sqrt(d.x*d.x+d.y*d.y+d.z*d.z));m.addProperty("health",e.getHealth());m.addProperty("maxHealth",e.getMaxHealth());m.addProperty("vx",e.getDeltaMovement().x);m.addProperty("vy",e.getDeltaMovement().y);m.addProperty("vz",e.getDeltaMovement().z);a.add(m);}return a;}
 ...
 private record Control(double forward,double strafe,boolean jump,boolean sprint,boolean attack,float yaw,float pitch){static Control empty(){return new Control(0,0,false,false,false,Float.NaN,Float.NaN);}}
}
